...
public class JpaUserInfoRepository implements UserInfoRepository {
	...
	public UserInfo save(UserInfo userInfo) {
		/* Remove existing UserInfo objects with the same subject. */
		TypedQuery<DefaultUserInfo> query =
			manager.createNamedQuery("DefaultUserInfo.getBySubject",
				DefaultUserInfo.class);
		query.setParameter("sub", userInfo.getSub());

		for (DefaultUserInfo otherInfo : query.getResultList()) {
			manager.remove(otherInfo);
		}
